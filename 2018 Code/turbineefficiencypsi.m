function out= turbineefficiencypsi(phi, psi, stages, AR, S,g,G)
%finds losses for fixed phi and stages, loop in handoptimizer for multiple
%values for psi. Preferably input an array for AR rather than a single
%value
...
if length(AR)>1
figure(1)
scatter(psi,losteffTE(7),'b')
hold on
scatter(psi,losteffprofile(7),'r')
hold on
scatter(psi,losteffendwall(7),'g')
hold on
scatter(psi,losteff2nd(7),'c')
hold on
scatter(psi,lostefftip(7),'m')
legend('TE','Profile','Endwall','Secondary','Tip Clearance')
hold on
% hold off
xlabel('\psi')
ylabel('Lost Efficiency')
title('Lost Efficiencies (AR = 1.4)')

figure(2)
plot(AR, efftotal, 'DisplayName', num2str(psi))
hold on
...
figure(3)
plot(AR, Cx(1,:),'DisplayName', num2str(psi))
...
figure(6)
plot(AR, vol,'DisplayName', num2str(psi))
xlabel('Aspect Ratio')
ylabel('Volume (m^3)')
...
figure(4)
plot(AR, vol/(pi.*r.^3), 'DisplayName',num2str(psi))
hold on
plot([0 AR(end)], [10*r 10*r], '--','DisplayName',num2str(psi))
hold on
xlabel('AR')
...
figure(7)
scatter(psi,max(efftotal),'DisplayName', num2str(psi))
xlabel('Stages')
ylabel('\eta')
title('Maximum Efficiency at Different n_{stages}')
hold on


figure(8)
plot(AR, lostefftip, 'DisplayName', num2str(psi))
ylim([0 inf])
xlabel('AR')
ylabel('Lost Efficiency')
title('Tip Loss')
