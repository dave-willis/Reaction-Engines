...
%constant chord (as opposed to constant AR)
...
for i = 1:m
    for j = 1:stages
        Restages(:,:,j,i)=rhostageisen(:,:,j).*V2.*Cx(i)./mu;
...
for j = 1:stages
    for i = 1:m
        p(:,:,j,i) = (Z*Cx(i)./2)./(cosd(alpha2).^2.*abs(tand(alpha1)-tand(alpha2)));
...
for i=1:m
    endwall1(:,:,1,i) = 0.25.*Cdwall(:,:,1,i).*(V1.^3).*rhostageisen(:,:,1).*2.*pi.*(2.*r).*Cx(i)./Tstageisen(:,:,1);
    endwall3(:,:,1,i) = 0.25.*Cdwall(:,:,1,i).*(V3rel.^3).*rhostageisen(:,:,1).*2.*pi.*(2.*r).*Cx(i)./Tstageisen(:,:,1);
    endwallmid(:,:,1,i) = 0.5.*Cdwall(:,:,1,i).*((V2rel.^3+V2.^3)./2).*rhostageisen(:,:,1).*2.*pi.*(2.*r).*Cx(i)./Tstageisen(:,:,1);
    for j=2:stages
        endwall1(:,:,j,i) = endwall1(:,:,j-1,i)+0.25.*Cdwall(:,:,j,i).*(V1.^3).*rhostageisen(:,:,j).*2.*pi.*(2.*r).*Cx(i)./Tstageisen(:,:,j);
        endwall3(:,:,j,i) =endwall3(:,:,j-1,i)+ 0.25.*Cdwall(:,:,j,i).*(V3rel.^3).*rhostageisen(:,:,j).*2.*pi.*(2.*r).*Cx(i)./Tstageisen(:,:,j);
        endwallmid(:,:,j,i) =endwallmid(:,:,j-1,i)+ 0.5*Cdwall(:,:,j,i).*((V2rel.^3+V2.^3)./2).*rhostageisen(:,:,j).*2.*pi.*(2.*r).*Cx(i)./Tstageisen(:,:,j);
    end
end
...
for i = 1:m
    ...
    Ysec(:,:,1,i)=2*(0.375.*0.1336).*Cx(i).*(cosd(alpha2).^3).*((tand(alpha1)-tand(alpha2)).^2)./...
        (Hstage(:,:,1).*sqrt(cosd(alpha1)).*cosd(alpham));
    
    deltas2nd(:,:,1,i)=Ysec(:,:,1,i).*(hostage(1)-hstage(:,:,1))./(Tstageisen(:,:,1));
    for j=2:stages
        Ysec(:,:,j,i)=2*(0.375.*0.1336).*Cx(i).*(cosd(alpha2).^3).*((tand(alpha1)-tand(alpha2)).^2)./...
...
for j=1:stages
for i=1:m
    volstage(:,:,j,i) = pi*stagewidth(i)*((r+Hstage(:,:,j)./2).^2);   
end
end
vol=sum(volstage,3);
paretovol = vol;
...
for j=1:m
    store(n*n*(j-1)+1:n*n*(j),3)=repelem(Cx(j),n*n);
end
