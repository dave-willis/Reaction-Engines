%% Description
% Runs a Pareto Optimum Analysis for designs with non-constant mean
% radius.Plots Pareto Curve, stores ordered Pareto Set and does an Axial
% Force and Vessel thickness analysis
% Adjust the variation of deltahostage to alter radius variation
clear 
clc
%hold off
...
for z=40 %set as a range to plot multiple curves, single number for specific analysis
    for g =  0.0005; %0.0002:0.0002:0.001

stages=z;
%phi & psi required to be same size
psi= [0.8:0.05:2.4];    
...
%this is actually delta ho per stage
deltahostage = linspace(0.5*deltaho./(stages),(1.5.*deltaho)./stages,stages);

r=zeros(n,n,stages);
Vx=zeros(n,n,stages);
r(:,:,1) = ((-deltahostage(1)./(psi.*Omega.^2)).^(0.5));
Vx(:,:,1)=Omega.*r(:,:,1).*phi;

for j = 2:stages
deltahostage(j) = deltahostage(j-1)+deltahostage(j);
%enthalpy drop per stage sets radius and Axial Velocity
r(:,:,j)=((-deltahostage(j)+deltahostage(j-1))./(psi.*Omega.^2)).^(0.5);
Vx(:,:,j) = Omega.*r(:,:,j).*phi;
end
U =Omega.*r; %dim(r)
Hin=mdot./(rho.*2*pi.*r(:,:,1).*Vx(:,:,1)); 
...
V2=zeros(n,n,stages);
V3=zeros(n,n,stages);
V2rel=zeros(n,n,stages);
V3rel=zeros(n,n,stages);

for j=1:stages
V1(:,:,j) = Vx(:,:,j)./cosd(alpha1);
V2(:,:,j) = Vx(:,:,j)./cosd(alpha2);         %Stator exit velocity
V3(:,:,j) = Vx(:,:,j)./cosd(alpha3);
V2rel(:,:,j) = Vx(:,:,j)./cosd(alpha2rel);
V3rel(:,:,j) = Vx(:,:,j)./cosd(alpha3rel);
end
Toout = Toin + deltaho./cp;    %scalar
Toutisen = Toout - (V3(:,:,end).^2)./(2*cp); %dim(phi./psi)

Tin=Toin-0.5*(V1(:,:,1).^2)./cp; 
%--------------------------------------------------------------------------
%% Stage By Stage Quantities


Pin = Poin -0.5.*rho.*(V1(:,:,1).^2);  
%Poutisen = Pin.*((Toutisen./Tin).^(gam./(gam-1)));   

% All of these are at stage exit!:
...
for j=1:stages
hstage(:,:,j) = hostage(j)-0.5.*(V2(:,:,j).^2);   
Tstageisen(:,:,j) = Tin +((deltahostage(j)-0.5.*(V3(:,:,j).^2))./cp);
Pstageisen(:,:,j)= Pin.*((Tstageisen(:,:,j)./Tin).^(gam./(gam-1)));

rhostageisen(:,:,j) = Pstageisen(:,:,j)./(R*Tstageisen(:,:,j));
Hstage(:,:,1)=rho.*Hin./rhostageisen(:,:,1);   
Tostageisen(:,:,j)=Toin + deltahostage(j)./cp; %Tstageisen(:,:,j)+V3.^2./cp;
if j>1
Hstage(:,:,j)= rhostageisen(:,:,j-1).*Hstage(:,:,j-1)./rhostageisen(:,:,j);
end
M3relstage = (Vx(:,:,j)./cosd(alpha3rel))./sqrt(gam*R.*Tstageisen(:,:,j));
M3stage = V3(:,:,j)./sqrt(gam*R.*Tstageisen(:,:,j));    
end
...
for i = 1:m
    for j = 1:stages
        Restages(:,:,j,i)=rhostageisen(:,:,j).*V2(:,:,j).*Cx(:,:,j,i)./mu;
...
for j = 1:stages
    for i = 1:m
        ...
        deltasprofile(:,:,1,i)=zetaprofile(:,:,1,i).*0.5.*Vbar(:,:,1).^2./Tstageisen(:,:,1);
        if j>1
            deltasprofile(:,:,j,i)=zetaprofile(:,:,j,i).*0.5.*Vbar(:,:,j).^2./Tstageisen(:,:,j)+deltasprofile(:,:,j-1,i);
            %^Profile Loss
        end
    end
end
...
%need three separate terms for three components of endwall scrubbing loss
endwall1=zeros(n,n,stages,length(AR)); %stage entry
endwall3=zeros(n,n,stages,length(AR)); %stage exit
endwallmid=zeros(n,n,stages,length(AR)); %between rotor and stator blade rows

for i=1:length(AR)
    endwall1(:,:,1,i) = 0.25.*Cdwall(:,:,1,i).*(V1(:,:,1).^3).*rhostageisen(:,:,1).*2.*pi.*(2.*r(:,:,1)).*Cx(1,i)./Tstageisen(:,:,1);
    endwall3(:,:,1,i) = 0.25.*Cdwall(:,:,1,i).*(V3rel(:,:,1).^3).*rhostageisen(:,:,1).*2.*pi.*(2.*r(:,:,1)).*Cx(:,:,1,i)./Tstageisen(:,:,1);
    endwallmid(:,:,1,i) = 0.5*Cdwall(:,:,1,i).*((V2rel(:,:,1).^3+V2(:,:,1).^3)./2).*rhostageisen(:,:,1).*2.*pi.*(2.*r(:,:,1)).*Cx(:,:,1,i)./Tstageisen(:,:,1);
    for j=2:stages
        endwall1(:,:,j,i) = endwall1(:,:,j-1,i)+0.25.*Cdwall(:,:,j,i).*(V1(:,:,j).^3).*rhostageisen(:,:,j).*2.*pi.*(2.*r(:,:,j)).*Cx(:,:,j,i)./Tstageisen(:,:,j);
        endwall3(:,:,j,i) =endwall3(:,:,j-1,i)+ 0.25.*Cdwall(:,:,j,i).*(V3rel(:,:,j).^3).*rhostageisen(:,:,j).*2.*pi.*(2.*r(:,:,j)).*Cx(:,:,j,i)./Tstageisen(:,:,j);
        endwallmid(:,:,j,i) =endwallmid(:,:,j-1,i)+ 0.5*Cdwall(:,:,j,i).*((V2rel(:,:,j).^3+V2(:,:,j).^3)./2).*rhostageisen(:,:,j).*2.*pi.*(2.*r(:,:,j)).*Cx(:,:,j,i)./Tstageisen(:,:,j);
    end
end
...
if S > 0
    ...
    for i = 1:length(AR)
        for j =1:stages
            ...
            deltastip(:,:,1,i)=(((Vx(:,:,1)./cosd(alpha3rel)).^2).*fracleakagerot(:,:,1,i).*(1-tand(alpha2rel).*(sind(alpha3rel).^2)./tand(alpha3rel))...
                + (V2(:,:,1).^2).*fracleakagestat(:,:,1,i).*(1-tand(alpha1).*(sind(alpha2).^2)./tand(alpha2)))./Tstageisen(:,:,1);
        if j>1
            deltastip(:,:,j,i)=((Vx(:,:,j)./cosd(alpha3rel).^2).*fracleakagerot(:,:,j,i).*(1-tand(alpha2rel).*(sind(alpha3rel).^2)./tand(alpha3rel))...
                + (V2(:,:,j).^2).*fracleakagestat(:,:,j,i).*(1-tand(alpha1).*(sind(alpha2).^2)./tand(alpha2)))./Tstageisen(:,:,j)...
                +deltastip(:,:,j-1,i);
    ...
    deltastip=zeros(n,n,stages, length(AR));
    for i=1:m
        deltastip(:,:,1,i)=(Cdtip.*g.*Cx(:,:,1,i).*0.65).*((Vx(:,:,1)./cosd(alpha3rel)).^2)./(Hstage(:,:,1).*p(:,:,1,i).*cosd(alpha3rel).*Tstageisen(:,:,1))...
            +(Cdtip.*g.*Cx(:,:,1,i).*0.65).*(V2(:,:,1).^2)./(Hstage(:,:,1).*p(:,:,1,i).*cosd(alpha2).*Tstageisen(:,:,1));
        for j=2:stages
            deltastip(:,:,j,i)=deltastip(:,:,j-1,i)+Cdtip.*g.*Cx(:,:,j,i).*0.65.*(((Vx(:,:,j)./cosd(alpha3rel)).^2)./Hstage(:,:,j).*p(:,:,j,i).*cosd(alpha3rel).*Tstageisen(:,:,j))...
                +Cdtip.*g.*Cx(:,:,j,i).*0.65.*(V2(:,:,j).^2)./(Hstage(:,:,j).*p(:,:,j,i).*cosd(alpha2).*Tstageisen(:,:,j));
...
for j=1:stages
for i=1:m
    volstage(:,:,j,i) = pi*stagewidth(:,:,j,i).*((r(:,:,j)+Hstage(:,:,j)./2).^2);   
end
end
...
%reshape will order as: vary psi, vary phi, vary AR
...
figure(6)
scatter(store2(:,5),store2(:,4),3, mkr(3),'DisplayName', num2str(stages))
hold on
 xlabel('Volume')
 ylabel('1 - \eta_{tt}')
 
%% Shape Plots
%plots to analyze 3 sets of optmised designs, most efficient, lowest volume
%and a third design in between these two
...
medianval= round(length(store3(:,1))./2); %replace by a number to analyse a specific design
...
figure(11)
plot(1:stages, ttetowplot(:,1), 'DisplayName', 'Increasing r_{mean}')
ylabel('t_{te}/w')
xlabel('Stages through Turbine')
hold on

figure(12)
scatter(stages, sum(nbladeplot(:,1)), 'DisplayName', num2str(z))
title('n_{blades}')
hold on
...
Pstagerotor(:,:,1)=Poin-0.5.*rho.*V2(:,:,1).^2; %evaluated at stage entry
...
Fx = zeros(n,n,stages);
Fx(:,:,1)= 2.*pi.*r(:,:,1).*(Pstageisen(:,:,1).*Hstage(:,:,1)-(Pstagerotor(:,:,1).*Hin+...
    (Pstagerotor(:,:,1)+Pstageisen(:,:,1))./2.*(Hstage(:,:,1)-Hin)));
for j = 2:stages
    Postageisen(:,:,j) = Postageisen(:,:,j-1).*((Tostageisen(:,:,j)./Tostageisen(:,:,j-1)).^(gam./(gam-1)));
    Pstagerotor(:,:,j) = Postageisen(:,:,j-1)-0.5.*rhostageisen(:,:,j-1).*V2(:,:,j).^2;
    Fx(:,:,j) =  2.*pi.*r(:,:,j).*(Pstageisen(:,:,j).*Hstage(:,:,j)) - 2.*pi.*r(:,:,j-1).*(Pstagerotor(:,:,j).*Hstage(:,:,j-1))-...
        ((Pstagerotor(:,:,j)+Pstageisen(:,:,j))./2).*(2.*pi.*(r(:,:,j).*Hstage(:,:,j)-r(:,:,j-1).*Hstage(:,:,j-1))) + mdot.*(Vx(:,:,j)-Vx(:,:,j-1));
end
